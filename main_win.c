...
int main() {
    ...
    while (!authenticated) {
        ...
        printf("Password: ");
        if (fgets(passBuffer, sizeof(passBuffer), stdin) == NULL) continue;
        passBuffer[strcspn(passBuffer, "\r\n")] = 0;
...
void cmd_help() {
    printf("\nCitron CLI v2.5 - 52 Commandes Supportees :\n");
    ...
    printf(" 51. filegui    52. exit/clear\n\n");
}
...
void cmd_pwdmod() {
    ...
    if (fgets(oldPass, sizeof(oldPass), stdin) == NULL) return;
    ...
    if (fgets(newPass, sizeof(newPass), stdin) == NULL) return;
    newPass[strcspn(newPass, "\r\n")] = 0;

    printf("Confirmer le mot de passe : ");
    if (fgets(confirmPass, sizeof(confirmPass), stdin) == NULL) return;
...
void cmd_sudosu() {
    ...
    if (fgets(passInput, sizeof(passInput), stdin) == NULL) return;
